...
//if(transpStateJacobian!=null) {
    ...
    public void init(Track trkcand, KFitter kf, DCSwimmer swimmer) {
        ...
        if (trkcand != null) {
            ...
            StateVec initSV = new StateVec(0);
            initSV.x = trkcand.get_Vtx0().x();//get_Vtx0().z(), TrackArray[i].get_pAtOrig().x()
            initSV.y = trkcand.get_Vtx0().y();
            initSV.z = trkcand.get_Vtx0().z();
            initSV.tx = trkcand.get_pAtOrig().x() / trkcand.get_pAtOrig().z();
            initSV.ty = trkcand.get_pAtOrig().y() / trkcand.get_pAtOrig().z();
            double p = trkcand.get_pAtOrig().mag();
            initSV.Q = (double)trkcand.get_Q() / p; 
            this.trackTraj.put(0, initSV);
            
            Matrix initCMatrix = F(0,1,initSV);
    ...
    public void setFittedTrackPars(Track trk, StateVec finalSV) {
        ...
        trk.set_Vtx0(new Point3D(xOrFix, yOrFix, zOrFix) );
        trk.set_pAtOrig(new Vector3D(pxOrFix, pyOrFix, pzOrFix) );
